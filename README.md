...
After activation, open `COLT Experience > Product CRM` in WordPress admin to manage WooCommerce products in bulk. The CRM includes product creation and editing in in-page modals, XLSX imports with column-to-field mapping and exact-name/SKU/ID matching, product search and filters, time-scoped stats, quick stock and price actions, category/tag bulk edits, bulk trash/permanent delete, display image metadata, coupon creation, active 3+1 promo rules, backorder signup tracking, sales insights, low-stock alerts, and customer summaries.
...
- Adds a WooCommerce Product CRM admin screen for product creation/editing, XLSX import mapping, bulk product operations, deletion, coupon creation, 3+1 promo rules, backorder signup tracking, and time-scoped store insights.
